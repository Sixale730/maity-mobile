...
class _DeveloperSettingsPageState extends State<DeveloperSettingsPage> {
  ...
  Widget _buildSttChip() {
    ...
    final label = useCustom ? SttProviderConfig.get(config.provider).displayName : 'Maity';
                              ...
                              Expanded(
                                child: GestureDetector(
                                  onTap: () async {
                                    ...
                                    if (files.length == 1) {
                                      final result =
                                          await Share.shareXFiles([XFile(files.first.path)], text: 'Maity debug log');
                                    ...
                                    if (selected != null) {
                                      final result =
                                          await Share.shareXFiles([XFile(selected.path)], text: 'Maity debug log');
                                      if (result.status == ShareResultStatus.success) {
                                        debugPrint('Log shared');
                                      }
                                    }
